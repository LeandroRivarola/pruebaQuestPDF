...
namespace QuestPDF.ExampleInvoice
{
    public class InvoiceDocument : IDocument
    {
        ...
        public FinancieroDataModel FinancieroModel { get;  }

        public InvoiceDocument(InvoiceModel model, FinancieroDataModel financieroModel)
        {
            FinancieroModel = financieroModel;
            Model = model;
        }
        ...
        void ComposeContent(IContainer container)
        {
            container.PaddingVertical(40).Column(column => 
            {
                ...
                column.Item().Row(row =>
                {
                    row.RelativeItem().Component(new AddressComponent("From", FinancieroModel.sucursal));
                    row.ConstantItem(50);
    ...
    public class AddressComponent : IComponent
    {
        ...
        private SucursalModel Address { get; }

        public AddressComponent(string title, SucursalModel address)
        {
            Title = title;
            Address = address;
        }
        ...
        public void Compose(IContainer container)
        {
            container.ShowEntire().Column(column =>
            {
                ...
                column.Item().Text(Address.nombre);
                column.Item().Text(Address.calle);
                column.Item().Text($"{Address.calleNumero}, {Address.callePiso}");
                column.Item().Text("lrivarola@eiva.com");
                column.Item().Text("341666333");
            });
        }
    }
}